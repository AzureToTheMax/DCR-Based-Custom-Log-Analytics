...
1.3 2026-01-17
- Updated to include Appx apps
- I am doing a major version increment to better handle the workbook queries as I can adjust $ScriptVersion
...
$AppInventory_Table = "AppInventoryV2_CL"
...
#Export to JSON for DCR Creation - Should be false for deployment.
$WriteLogFile = $false
...
if($CollectDeviceInventory) {
	...
	#Check SMBv1 Status
	#Old method which just checked feature presence - $SMBv1Status = (Get-WindowsOptionalFeature -Online -FeatureName SMB1Protocol).state

    #Get the mrxsmb10 key
    $Mrxsmb10Status = Get-ItemProperty "HKLM:\SYSTEM\CurrentControlSet\services\mrxsmb10" -ErrorAction SilentlyContinue
    #Start should be set to 4 (disabled)

    #Get the LanmanWorkstation key
...
if ($CollectAppInventory) {
	...
	foreach ($App in $CleanAppList) {
		...
		$tempapp | Add-Member -MemberType NoteProperty -Name "AppType" -Value "Classic" -Force
	...
	if ($CollectAppxInventory -eq $true){
	#Gather appx package installs
	$InstalledAppxPackages = Get-AppxPackage -AllUsers 


	$FinalAppxAppList = $InstalledAppxPackages

	$FinalAppxAppList | ForEach-Object -Process {
		$tempapp = New-Object -TypeName PSObject

		#Get the installing users which need to be translated from SID and summarized
		

		<#
		#Forgotten section looking into the possibility of stripping out duplicates based on user.
		$UniqueAppxApps = ($InstalledAppxPackages | Group-Object Name | Where-Object { $_.Count -eq 1 }).Group
		$DuplicatedAppxApps = ($InstalledAppxPackages | Group-Object Name | Where-Object { $_.Count -gt 1 }).Group
		$MergedAppxApps = ($DuplicatedAppxApps | Group-Object Name) | ForEach-Object { $_.Group | Sort-Object [version]Version -Descending | Select-Object -First 1 }
		$FinalAppxAppList = $UniqueAppxApps + $MergedAppxApps | Sort-Object DisplayName
		#>

		$AppxInstallingUsers = @()
		#$UserCount = 0 #Now unused.
		$_.PackageUserInformation | ForEach-Object -Process {
		
			if ($_.InstallState -eq "Installed"){
				$AppxTempUser = ($_ -split ' ')[0]
				$AppxTempSID = ($_ -split ' ')[0]

				#Thank you Joe
				#https://joestechspace.com/blog/how-to-convert-a-sid-to-username-in-powershell

				#While there is a more complicated process to translate any sid via Entra graph lookup (https://intunediff.com/tools/entra-id-converter) that requires permissions and I don't feel like writing a function app just to check this. Translate when you need it!
				$SID = New-Object System.Security.Principal.SecurityIdentifier($AppxTempUser)

				try {
					#Strip off leading AzureAD\, NT Authority\, etc
					[String]$AppxTempUserName = ($SID.Translate([System.Security.Principal.NTAccount])).Value -creplace '^[^\\]*\\', ''

					#If no stripping is wanted
					#$AppxTempUserName = ($SID.Translate([System.Security.Principal.NTAccount])).Value
					
				}
				catch {
					#If it fails to translate the SID, use the SID.
					$AppxTempUserName = $AppxTempSID
				}

				#failed method of building this data into an array. This would have each object under it's own line which is a mess.
				#$TempAppxUser = new-object -TypeName PSObject
				#$TempAppxUser | Add-Member -MemberType NoteProperty -Name "User-$($UserCount)" -Value "$AppxTempUserName" -force
				#$UserCount = $UserCount + 1
				
				$AppxInstallingUsers += $AppxTempUserName
				}
		}

		#[System.Collections.ArrayList]$AppxInstallingUsersArray = $AppxInstallingUsers #failed method of building this data into an array.
		if ('' -eq $AppxInstallingUsers){$AppxInstallingUsers += "None"} #Set it to "none", rather than a blank

		[string]$AppxInstallingUsersFinal = $AppxInstallingUsers -join ","


		$tempapp | Add-Member -MemberType NoteProperty -Name "ComputerName" -Value "$ComputerName" -Force
		$tempapp | Add-Member -MemberType NoteProperty -Name "ManagedDeviceName" -Value "$ManagedDeviceName" -Force
		$tempapp | Add-Member -MemberType NoteProperty -Name "ManagedDeviceID" -Value "$ManagedDeviceID" -Force
		$tempapp | Add-Member -MemberType NoteProperty -Name "AppName" -Value $_.Name -Force
		$tempapp | Add-Member -MemberType NoteProperty -Name "AppVersion" -Value $_.Version -Force
		$tempapp | Add-Member -MemberType NoteProperty -Name "AppPublisher" -Value ($_.Publisher -replace 'CN=|,.+')  -Force
		$tempapp | Add-Member -MemberType NoteProperty -Name "AppType" -Value "Appx" -Force
		$tempapp | Add-Member -MemberType NoteProperty -Name "AppUninstallString" -Value $_.PackageFullName -Force
		$tempapp | Add-Member -MemberType NoteProperty -Name "AppInstallLocation" -Value $_.InstallLocation -Force
		$tempapp | Add-Member -MemberType NoteProperty -Name "Architecture" -Value ([string]$_.Architecture) -Force
		$tempapp | Add-Member -MemberType NoteProperty -Name "InstalledUsers" -Value $AppxInstallingUsersFinal -Force
		$tempapp | Add-Member -MemberType NoteProperty -Name "ScriptVersion" -Value "$ScriptVersion" -Force
		$tempapp | Add-Member -MemberType NoteProperty -Name "UploadGUID" -Value $UploadGUIDGUID.guid -Force
